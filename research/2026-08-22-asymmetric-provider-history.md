...
LAB-082 is now integrated behind the existing LAB-080 SQLite serialization boundary rather than as a parallel authority store.
...
A final evidence-semantics audit found a separate downgrade: stable request identity alone did not distinguish a signed `READ` from signed `RECONCILE` evidence. The supported boundary now requires `kind == RECONCILE` before historical evidence can justify a confirmed ledger effect. The request-level stable binding intentionally remains challenge-independent so independent reconciliation workers can converge on the same logical effect identity.
...
Exact executable bytes were reconstructed through the GitHub connector and verified against Git blob identities before execution because direct shell DNS access to `github.com` was unavailable in the validation runtime.

Observed results:

- LAB-082 corrected suites: 28/28 passed;
- LAB-081 regressions: 20/20 passed;
- LAB-080 + LAB-036 regressions: 30/30 passed;
...
- compileall passed for LAB-082/LAB-081/LAB-080/LAB-036 sources.
