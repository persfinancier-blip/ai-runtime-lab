...
Transferable mechanism: completion is evaluated by a harness against repository state and tests, not by asking the agent whether the issue was solved. The useful runtime principle is to make terminal success a verifier decision over observed artifacts/results.
...
Transferable mechanisms: provenance identifies the subject artifact and how it was produced; verification checks that provenance applies to the artifact and matches expectations. Evidence for artifact A must not silently validate mutated artifact B.
...
Transferable mechanism: separate a statement's subject from its predicate/evidence and use stable artifact identity/digests so downstream verification can decide whether an assertion applies to the object under evaluation.
...
Acceptance requires supported schema, complete requirement coverage, resolvable evidence references, actually observed evidence, matching artifact digest, passing evidence for every requirement, and at least one current observed passing test.

## Experiment

Local implementation was executed before publication. Seven deterministic trajectories passed their expected accept/reject assertions, and Python compile validation passed.

Seeded trajectories: correct completion accepted; unexecuted test rejected; failing observed test rejected; stale artifact-bound evidence rejected; partial completion rejected; nonexistent evidence reference rejected; mutation invalidating formerly valid evidence rejected.
...
It cannot prove that requirements are complete/correct, that passing tests have adequate semantic coverage, that an evidence producer is trustworthy if it can forge the observation channel, that nondeterministic behavior remains correct, or human/product judgments without machine-checkable criteria.
