...
class IntegrationTests(unittest.TestCase):
    ...
    def test_reservation_rotation_race_has_no_historical_prepared_intent(self):
        for _ in range(10):
            with tempfile.TemporaryDirectory() as td:
                ...
                def reserve():
                    barrier.wait()
                    try: outcomes.append(("reserve", ledger.reserve(self.intent("race"))))
                    except Exception as exc: outcomes.append(("reserve-error", exc))

                def rotate():
                    barrier.wait()
                    try: outcomes.append(("rotate", ledger.rotate_provider(self.s2, AsymmetricProviderHistory.make_transition(self.s1, self.s2), a2)))
                    except Exception as exc: outcomes.append(("rotate-error", exc))

                t1 = threading.Thread(target=reserve); t2 = threading.Thread(target=rotate)
                t1.start(); t2.start(); barrier.wait(); t1.join(5); t2.join(5)
                self.assertFalse(t1.is_alive() or t2.is_alive())
                entry = ledger.entry("race"); head = ledger.provider_history.current()
    ...
    def test_local_snapshot_rollback_is_not_claimed_as_external_freshness_proof(self):
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "shared.db"; snapshot = Path(td) / "pre-rotation.db"
            _, a1 = self.attested(1, b"h1", 0)
            ledger = AsymmetricHistoricalSharedAnchorLedger(path, a1, self.s1.public, self.s1)
            ledger.execute(self.intent("one"))
            source = sqlite3.connect(path); target = sqlite3.connect(snapshot)
            try: source.backup(target)
            finally: target.close(); source.close()
            _, a2 = self.attested(2, b"h2", 1)
            ledger.rotate_provider(self.s2, AsymmetricProviderHistory.make_transition(self.s1, self.s2), a2)
            rolled = Path(td) / "rolled.db"; rolled.write_bytes(snapshot.read_bytes())
