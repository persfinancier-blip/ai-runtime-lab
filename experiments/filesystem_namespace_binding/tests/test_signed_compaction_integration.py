...
class SignedCompactionNamespaceIntegrationTests(unittest.TestCase):
    ...
    def test_swap_after_authorization_cannot_redirect_and_blocks_sql_commit(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # Publication happened, if at all, only through the held original FD.
    ...
    def test_swap_after_publication_receipt_fails_before_sql_commit(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            attacker = td / "attacker"
            attacker.mkdir()
            published = {"archive_id": None}

            def swap(_handle, manifest):
                published["archive_id"] = manifest.archive_id
                archive.rename(original)
                os.symlink(attacker, archive, target_is_directory=True)
            ...
            with self.assertRaises(NamespaceMismatch):
                layer.compact(checkpoint)
            self.assert_uncompacted(layer)
            aid = published["archive_id"]
            self.assertIsNotNone(aid)
            self.assertTrue((original / f"{aid}.json").exists())
            self.assertTrue((original / f"{aid}.manifest.json").exists())
            self.assertEqual(list(attacker.iterdir()), [])
    ...
    def test_directory_identity_is_stable_across_rename_without_path_retarget(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                before = handle.directory
                archive.rename(td / "renamed")
                self.assertEqual(handle.directory, before)
                receipt = handle.publish("probe.bin", b"probe")
                self.assertTrue(receipt.durable)
                self.assertTrue((td / "renamed" / "probe.bin").exists())
            finally:
                handle.close()
