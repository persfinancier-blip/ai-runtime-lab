...
from experiments.provider_threshold_rotation.protocol import (
    ...
    StaleAuthority, ThresholdNotMet, ThresholdProof, mac, sha, verify_threshold,
)
...
class SupportedRecoveryThresholdProviderLedger(SupportedThresholdAuthorizedAsymmetricProviderLedger):
    ...
    def _reject_prepared_locked(q):
        if q.execute("SELECT COUNT(*) FROM shared_anchor_intents WHERE status='PREPARED'").fetchone()[0]:
    ...
    def rotate_rotation_authority(self, new_authority: RotationAuthority, old_signatures: tuple[Signature, ...], new_signatures: tuple[Signature, ...]):
        ...
        try:
            ...
            result = self.rotation_authority.rotate_authority_locked(q, new_authority, old_signatures, new_signatures)
    ...
    def recover_rotation_authority(self, new_authority: RotationAuthority, recovery_signatures: tuple[Signature, ...]):
        ...
        try:
            q.execute("BEGIN IMMEDIATE")
            self._reject_prepared_locked(q)
            result = self.recovery.recover_locked(q, new_authority, recovery_signatures)
            q.commit(); return result
        except:
            if q.in_transaction: q.rollback()
            raise
    ...
    def _verify_normal_edge_locked(self, q, old, new):
        ...
        for authority, raw in ((old, row[2]), (new, row[3])):
            ...
            if valid < authority.threshold:
                raise ThresholdNotMet("persisted normal authority transition below threshold")
    ...
    def _verify_mixed_authority_history_locked(self, q, provider_transitions):
        ...
        normal_total=q.execute("SELECT COUNT(*) FROM provider_rotation_authority_transitions").fetchone()[0]
        recovery_total=q.execute("SELECT COUNT(*) FROM provider_rotation_recovery_transitions").fetchone()[0]
        if normal_total + recovery_total != len(authorities)-1:
        ...
        for old,new in zip(authorities,authorities[1:]):
            ...
            recovery=q.execute("SELECT COUNT(*) FROM provider_rotation_recovery_transitions WHERE new_rotation_authority_id=?",(new.authority_id,)).fetchone()[0]
            ...
            else: self.recovery.verify_recovery_transition_locked(q,old,new)
        ...
        for provider_id,old_gid,new_gid in provider_transitions:
            ...
            proof=ThresholdProof(row[3],row[0],row[1],row[2],self.rotation_authority._decode_signatures(row[4]))
            verify_threshold(authority,intent,proof)
        return head

    def verify_durable(self):
        # Bypass LAB-083's normal-only authority verifier, but retain LAB-082 history verification.
