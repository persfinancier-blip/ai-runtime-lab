...
class KernelAdapter:
    ...
    def __init__(self, defect: str | None = None):
        ...
        self.db = Path(self._tmp.name) / "kernel.db"
        self.kernel = Kernel(self.db)
        self.kernel.ensure_work(WORK_ID)
        self.fence = 0
        self.effect_count = 0
        self.external_receipt: str | None = None
    ...
    def observe(self) -> State:
        row = self.kernel.state(WORK_ID)
        evidence, evidence_valid = self._evidence()
        if row["phase"] == "DONE":
            self.ever_done = True
        return State(
    ...
    def step(self, action: str) -> State:
        ...
        try:
            if action == "claim":
                self.fence, _ = self.kernel.claim(WORK_ID, OWNER)
            elif action == "intent":
                self.kernel.prepare_intent(WORK_ID, OWNER, self.fence, EFFECT_KEY)
                ...
                if before.intent and before.phase in ("INTENT", "UNKNOWN"):
                    if self.effect_count == 0:
                        ...
                        self.effect_count += 1
                    if action == "effect_ok":
                        self.kernel.confirm_effect(WORK_ID, OWNER, self.fence, self.external_receipt or "receipt-1")
                    else:
                        self.kernel.mark_unknown(WORK_ID, OWNER, self.fence)
            elif action == "reconcile":
                if before.phase == "UNKNOWN" and self.external_receipt:
                    self.kernel.confirm_effect(WORK_ID, OWNER, self.fence, self.external_receipt)
            elif action == "append_evidence":
                if before.confirmed and before.phase != "DONE":
                    self.kernel.append_evidence(WORK_ID, EVIDENCE_ID, "v1")
            elif action == "invalidate":
                if self.defect == "terminal_invalidation" and before.phase == "DONE":
                    self._raw("UPDATE evidence SET valid=0 WHERE evidence_id=?", (EVIDENCE_ID,))
                else:
                    self.kernel.invalidate(EVIDENCE_ID)
                ...
                elif before.confirmed and before.evidence and before.evidence_valid:
