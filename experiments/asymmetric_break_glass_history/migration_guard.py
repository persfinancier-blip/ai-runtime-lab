...
def migration_payload(
    *, legacy_digest, cutoff_root, symmetric_authority_id, public_authority
    ...
    return {
        "kind": "provider-asymmetric-break-glass-boundary-v2",
...
class AuthenticatedBreakGlassMigrationGuard:
    def __init__(self, ledger):
        ...
        try:
            ...
            # Before migration, the compatibility HMAC history must still pass the
            # full LAB-085 verifier. After migration those proof bytes are scrubbed
            # intentionally, so only the authenticated cutoff/public history is
            # authoritative for legacy break-glass edges.
    ...
    def _legacy_snapshot_locked(self, q, cutoff_version):
        # The signed migration snapshot deliberately commits the semantic identity
        # of every legacy recovery edge but not the HMAC proof bytes themselves.
        # Full LAB-085 HMAC verification happens immediately before the boundary is
        # signed. Once signed, those MAC bytes can be destroyed without weakening
        # the exact predecessor/successor/recovery binding committed here.
        rows = q.execute(
            ...
            "r.recovery_generation,r.intent_digest "
        ...
        for row in rows:
            custody = q.execute(
                ...
                "custody_intent_digest,public_signatures_json "
                "FROM provider_rotation_recovery_custody_proofs WHERE new_rotation_authority_id=?",
                (row[0],),
            ).fetchone()
            out.append(
                {
                    "legacy_recovery_edge": list(row),
    ...
    @staticmethod
    def _scrub_legacy_hmac_proofs_locked(q, cutoff_version):
        q.execute(
            "UPDATE provider_rotation_recovery_transitions SET signatures_json='[]' "
            "WHERE new_rotation_authority_id IN "
            "(SELECT authority_id FROM provider_rotation_authorities WHERE version<=?)",
            (cutoff_version,),
        )

    @staticmethod
    def _assert_legacy_hmac_proofs_scrubbed_locked(q, cutoff_version):
        count = q.execute(
        ...
        if count:
            raise MigrationGuardError(
                "legacy HMAC break-glass proof bytes remain after migration"
            )
        return True
    ...
    def _current_components_locked(self, q):
        ...
        compat = self.ledger.recovery.current_recovery_locked(q)
    ...
    def _verify_boundary_recovery_window_locked(self, q, root, symmetric):
        ...
        window = self.ledger._lifecycle_windows_locked(q, roots).get(
            symmetric.recovery.authority_id
        )
        if window is None:
            raise MigrationGuardError("boundary references unknown recovery generation")
        versioned, lower, upper = window
        if versioned.authority_id != symmetric.authority_id:
            raise MigrationGuardError("boundary recovery lifecycle identity mismatch")
        if lower is not None and root.version < lower:
            raise MigrationGuardError(
                "boundary recovery generation used before activation"
            )
        if upper is not None and root.version >= upper:
            raise MigrationGuardError(
                "stale recovery generation cannot authorize migration boundary"
            )
        return True
    ...
    def establish(self, public_signatures):
        ...
        try:
            ...
            if q.execute(
                ...
                raise MigrationGuardError("migration boundary already exists")
            self.ledger._reject_prepared_locked(q)
            # This is the last point at which legacy HMAC proof bytes are trusted.
            # Verify the complete LAB-085 history before signing the projection.
            self._verify_preboundary_locked(q)
            root, symmetric, public = self._current_components_locked(q)
            legacy = self._legacy_digest_locked(q, root.version)
            ...
            # Boundary insertion and HMAC proof destruction are one atomic commit:
            # crash leaves either the verified pre-migration history or a complete
            # signed boundary with no retained legacy MAC proof bytes.
            self._scrub_legacy_hmac_proofs_locked(q, root.version)
            self.verify_locked(q)
            q.commit()
            return bd
        except:
    ...
    def verify_locked(self, q):
        ...
        if row is None:
            return None
        legacy, rid, rv, rg, sid, pid, pv, pg, bd, sigs = row
        ...
        symmetric = self.ledger.recovery_lifecycle._load_recovery_locked(q, sid)
        ...
        if binding != (public.authority_id, symmetric.version, symmetric.generation):
            raise MigrationGuardError(
                "boundary recovery authority is not historically bound"
            )
        self._verify_boundary_recovery_window_locked(q, root, symmetric)
        payload = migration_payload(
            legacy_digest=legacy,
            cutoff_root=root,
            symmetric_authority_id=sid,
            public_authority=public,
        )
        ...
        if self._legacy_digest_locked(q, root.version) != legacy:
            raise LegacyHistoryChanged("legacy HMAC history changed after migration")
        self._assert_legacy_hmac_proofs_scrubbed_locked(q, root.version)
        return {
            "boundary_digest": bd,
            "legacy_digest": legacy,
