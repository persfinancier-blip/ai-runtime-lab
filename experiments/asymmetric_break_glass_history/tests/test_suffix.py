...
class AsymmetricSuffixIntegrationTests(unittest.TestCase):
    def make_ledger(self, path):
        ...
        enable = ThresholdEnablement(
            base.start_provider_generation_id, 1, root.authority_id, 1, 1,
        ...
        ledger = SupportedAsymmetricBreakGlassLedger(
            ...
            custody_enablement_signatures=public_signatures(public_signers, enable_payload, 3),
        )
    ...
    def migrate(self, ledger, public_signers):
        ...
        return ledger.migration_guard.establish(public_signatures(public_signers, payload, 3))

    def test_post_cutoff_recovery_uses_only_asymmetric_proof_and_restarts(self):
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "db"
            ledger, signer, root1, rec1, _, public1, public_signers, enable = self.make_ledger(path)
            ...
            self.assertEqual(q.execute("SELECT COUNT(*) FROM provider_asymmetric_break_glass_proofs").fetchone()[0], 1)
            q.close()
            ...
            self.assertEqual(restarted.rotation_authority.current().authority_id, root2.authority_id)

    def test_hmac_compatibility_entry_points_are_blocked_after_migration(self):
        with tempfile.TemporaryDirectory() as td:
            ledger, _, _, _, _, _, public_signers, _ = self.make_ledger(Path(td) / "db")
            self.migrate(ledger, public_signers)
    ...
    def test_asymmetric_proof_tamper_fails_restart(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            payload = ledger.asymmetric_recovery_payload(root2)
            ledger.recover_rotation_authority_asymmetric(root2, public_signatures(public_signers, payload, 3))
            q = sqlite3.connect(path)
            q.execute("UPDATE provider_asymmetric_break_glass_proofs SET public_signatures_json='[]'")
            q.commit(); q.close()
    ...
    def test_exactly_one_root_proof_type_is_required(self):
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "db"
            ledger, _, _, _, _, _, public_signers, _ = self.make_ledger(path)
            self.migrate(ledger, public_signers)
            root2, _ = authority(2, 2, "count")
            payload = ledger.asymmetric_recovery_payload(root2)
            ledger.recover_rotation_authority_asymmetric(root2, public_signatures(public_signers, payload, 3))
            q = sqlite3.connect(path)
            q.execute("INSERT INTO provider_rotation_authority_transitions VALUES(?,?,?,?,?)", (root2.authority_id, ledger.rotation_authority.bootstrap.authority_id, "0" * 64, "[]", "[]"))
            q.commit(); q.close()
            with self.assertRaises(Exception):
