...
This test-only module intentionally does not import production provenance code and does
not define precursor SQL DDL. It freezes only authority bytes whose schemas/domains
are already specified by durable design contracts.

The HMAC values below are prototype reference vectors only. They prove that predecessor
and successor authorities consume the exact same canonical precursor bytes while using
distinct keys. They do not select a future production signing primitive.

Nothing in this module is imported by production code. It deliberately does NOT satisfy
``lab099_precursor_fixture_adapter``'s DDL/mutation-plan contract.
"""
...
def _record(domain: str, fields: tuple[tuple[int, int, bytes], ...]) -> bytes:
    ...
    if ids != tuple(sorted(ids)):
        raise ValueError("reference fields must be strictly ordered")
    if len(set(ids)) != len(fields):
...
# Reference-only digest value. This does NOT freeze any precursor SQL/DDL spelling.
REFERENCE_ONLY_RELATION_DEFINITION_DIGEST = bytes(range(0xA0, 0xC0))
...
PREPARED_CANONICAL_BYTES = bytes.fromhex(
    ""
)
PREPARED_DIGEST = bytes.fromhex(
    "322ed4112534f6e159f70800d5e5eca2c2e71dec24cbdeca1f6865a4976009d7"
)
...
PREPARED_REFERENCE_VALUES = {
    ...
    6: 1,
    7: REFERENCE_ONLY_RELATION_DEFINITION_DIGEST,
    8: 1,
    9: 2,
    10: MIGRATION_NONCE,
}
...
CONFIRMED_CANONICAL_BYTES = bytes.fromhex(
    ""
)
CONFIRMED_DIGEST = bytes.fromhex(
    "d9032ca982484ab8db66fb7f0a68fcdde381dffaa3c031b2520b2170805a9620"
)
...
CONFIRMED_REFERENCE_VALUES = {
    ...
    5: 1,
    6: REFERENCE_ONLY_RELATION_DEFINITION_DIGEST,
...
def validate_reference_vectors() -> bool:
    """Recompute every frozen vector without calling future production LAB-099 code."""
    ...
    if hmac.new(
        REFERENCE_PREDECESSOR_KEY,
        REFERENCE_AUTHENTICATOR_INPUT,
        hashlib.sha256,
    ).digest() != REFERENCE_PREDECESSOR_HMAC_SHA256:
        raise AssertionError("predecessor prototype authenticator vector drift")
    if hmac.new(
        REFERENCE_SUCCESSOR_KEY,
        REFERENCE_AUTHENTICATOR_INPUT,
        hashlib.sha256,
    ).digest() != REFERENCE_SUCCESSOR_HMAC_SHA256:
        raise AssertionError("successor prototype authenticator vector drift")

    prepared = _prepared_from_values()
    if prepared != PREPARED_CANONICAL_BYTES:
        raise AssertionError("PREPARED canonical bytes drift")
    if hashlib.sha256(prepared).digest() != PREPARED_DIGEST:
        raise AssertionError("PREPARED digest drift")

    confirmed = _confirmed_from_values()
    if confirmed != CONFIRMED_CANONICAL_BYTES:
    ...
    if hashlib.sha256(confirmed).digest() != CONFIRMED_DIGEST:
        raise AssertionError("CONFIRMED digest drift")

    if PRECURSOR_DIGEST in {PREPARED_DIGEST, CONFIRMED_DIGEST}:
        raise AssertionError("domain-separated authority digests unexpectedly collide")
    return True
