...
class View:
 peer:str; events:tuple; signature:str
 @property
 ...
 def issue(cls,peer,events,key):
  u={"peer":peer,"events":list(events)}; return cls(peer,tuple(events),sig(key,u))
@dataclass(frozen=True)
class Obs:
 observer:str; seq:int; pred:str|None; peer:str; view_id:str; events:tuple; claimed_time:int; signature:str
 @property
 def unsigned(self): return {"observer":self.observer,"seq":self.seq,"pred":self.pred,"peer":self.peer,"view_id":self.view_id,"events":list(self.events),"claimed_time":self.claimed_time}
 @property
 def id(self): return dig({**self.unsigned,"signature":self.signature})
def rel(a,b):
 n=min(len(a),len(b))
...
class Store:
 ...
 def save(self,s):
  self.path.parent.mkdir(parents=True,exist_ok=True); fd,tmp=tempfile.mkstemp(dir=self.path.parent)
  with os.fdopen(fd,"w") as f: json.dump(s,f,sort_keys=True); f.flush(); os.fsync(f.fileno())
  os.replace(tmp,self.path)
 def load(self): return json.loads(self.path.read_text())
class Tracker:
 def __init__(self,peer_keys,observer_keys,store=None,quorum=2):
  self.pk=peer_keys; self.ok=observer_keys; self.store=store; self.quorum=quorum; self.s={"obs":[],"heads":{},"equiv":[]}
  if store and store.path.exists(): self.s=store.load(); self.verify()
 def persist(self):
  if self.store:self.store.save(self.s)
 def verify_view(self,v):
  if v.peer not in self.pk or not hmac.compare_digest(sig(self.pk[v.peer],v.unsigned),v.signature): raise Auth()
 def issue(self,observer,v,claimed_time=0):
  self.verify_view(v); h=self.s["heads"].get(observer); seq=1 if not h else h["seq"]+1; pred=None if not h else h["id"]
  u={"observer":observer,"seq":seq,"pred":pred,"peer":v.peer,"view_id":v.id,"events":list(v.events),"claimed_time":claimed_time}
  return Obs(observer,seq,pred,v.peer,v.id,v.events,claimed_time,sig(self.ok[observer],u))
 def dec(self,r): return Obs(r["observer"],r["seq"],r["pred"],r["peer"],r["view_id"],tuple(r["events"]),r["claimed_time"],r["signature"])
 def verify_obs(self,o):
  if o.observer not in self.ok or type(o.seq) is not int or o.seq<1 or not hmac.compare_digest(sig(self.ok[o.observer],o.unsigned),o.signature): raise Auth()
 def accept(self,o):
  ...
  self.s["obs"].append({**asdict(o),"events":list(o.events)}); self.s["heads"][o.observer]={"seq":o.seq,"id":o.id}; self.persist(); return self.classify(o.peer)
 def chains(self):
  d={}
 ...
 def verify(self):
  ...
  if heads!=self.s["heads"]: raise Auth()
  return True
class Unsafe:
