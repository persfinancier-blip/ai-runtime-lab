...
The exact published `strict_fence.py` blob `62a9b602edb8692894cad3874ba6d5c211129aa5` matched the locally executed bytes. The exact published regression blob `7a2ce4ed521ce523250c4331ab14f1847d322d6a` was reconstructed byte-for-byte and passed 3/3; compileall passed. The new regression creates historical rows using the transaction-scoped final-writer path, reinstalls the fence, then proves UPDATE and DELETE fail with the original rows unchanged.

## Evidence produced / reconfirmed

- Exact current-head inherited-history fence suite: 3/3 PASS.
- Exact current-head focused compileall for the hardened fence/test: PASS.
- Pre-fix executable counterexample: UPDATE and DELETE succeeded on all three inherited history tables.
- Current `strict_fence.py` blob: `62a9b602edb8692894cad3874ba6d5c211129aa5`.
- Current `test_inherited_sql_fence.py` blob: `7a2ce4ed521ce523250c4331ab14f1847d322d6a`.
...
- The focused 3/3 result is exact current-head evidence for the inherited-history DML fence only; it is not the full merged-stack gate.
...
2. Re-fetch PR #165 HEAD (do not assume `d467c050...` remains current), reconstruct current LAB-086 implementation/tests and execute the complete real-schema suite, explicitly including the new inherited UPDATE/DELETE regression, public-custody history guard, root-head REPLACE/DELETE, migration v4 root coauthorization/restart, scrubbed-prefix/asymmetric-suffix, forged-proof/stale-writer/direct-surface, strict conflict algorithms, final verification snapshot, public-rotation history guard and rotation races.
...
- #163 / LAB-086 — IN_PROGRESS; inherited historical DML UPDATE/DELETE blocker fixed; full current-head gate remains.
