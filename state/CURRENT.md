...
Fallback source audit of PR #177 found a third LAB-092 provenance TOCTOU in the explicit migration transition itself. `migrate_activation_schema_v1()` first commits exact activation DDL + deterministic PREPARED completion marker inside `_install_and_reserve_prepared()`, releases that transaction, then creates an unbound confirmation surface and eventually calls inherited `confirmation.execute()`. If a second SQLite actor deletes/changes activation DDL after the PREPARED commit but before the consequential portion of `execute()`, inherited execution can advance/reconcile the external anchor, persist historical receipt evidence, and flip the migration marker to CONFIRMED without proving the DDL is still exact at that authority boundary. The later `cls(...)` startup may fail closed, but that is too late: a false authenticated migration-completion claim/provider advance may already exist. This strengthens #176; no duplicate issue was created.
...
- LAB-092 provenance checks must be bound to the consequential mutation/serialization boundary during explicit migration completion, startup confirmation, and post-construction operations; another unsynchronized `_classify()` merely moves the TOCTOU window.
...
If exact source execution becomes available first, run retained LAB-090 REDs before PR #175 production changes; then run three LAB-092 deterministic TOCTOU RED groups: (1) explicit migration — pause after `_install_and_reserve_prepared()` commits exact DDL+PREPARED (and separately after read-only confirmation checks but before the first consequential action in `execute()`), mutate/delete activation DDL from a second connection, and require failure before provider position/request-results, historical receipt, CONFIRMED marker, watermark, provider-history mutation, or further durable advancement; (2) constructor/restart — pause after initial COMPLETE classification/authority checks but before `confirmation.execute`, mutate marker or activation DDL from a second connection, and require the same fail-before-mutation property; (3) post-construction — pause `execute()` immediately after successful reserve/provenance check, delete the confirmed marker from a second connection, resume, and require fail-closed before further external/durable mutation, extending to watermark commit and provider rotation. Then run retained LAB-100 provider REDs.
...
- #176 / LAB-092 — IN_PROGRESS; exact regression/full gate pending; explicit-migration, constructor/restart, and post-construction provenance TOCTOU regressions are all required.
