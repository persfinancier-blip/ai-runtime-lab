...
Fallback source audit returned to exact PR #175 and found an additional pre-durability orphan-reservation path. After `provider.prepare_activation(...)` succeeds and installs the provider-owned pending fence, `rotate_provider()` validates the returned ticket and then calls `provider.activation_status(ticket)` before entering the later SQLite `q = self._con()` / cleanup scope. If that status probe raises — especially once unavailable-provider status is corrected to fail closed — the method exits with no durable activation row and no abort/recovery attempt. This is earlier than, and independent from, the already-recorded post-prepare connection-open leak. Successful prepare must therefore enter one coherent ownership/cleanup-or-durable-recovery scope immediately, before any fallible status/ticket/SQLite operation.
...
- #169 / LAB-090 — IN_PROGRESS; exact behavioral/full gate pending; outage semantics and post-prepare ownership-scope regressions expanded.
