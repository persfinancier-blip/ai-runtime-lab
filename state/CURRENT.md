...
Fallback source audit of PR #177 found a distinct LAB-092 constructor/restart provenance TOCTOU. `ProvenancedHistoricalSharedAnchorLedger.__init__()` checks `_classify(path) == COMPLETE`, then builds an unbound LAB-090 confirmation object and calls `confirmation.execute(_completion_intent())` before `super().__init__()` and before the live provider-history handle is provenance-bound. If the confirmed migration marker or activation DDL is changed after the initial classification, inherited `execute()` can perform provider I/O and/or persist a historical receipt or replacement marker before later construction fails closed. Provenance loss must instead be detected before any consequential external/durable authority mutation. This strengthens #176; no duplicate issue was created.
...
- LAB-092 provenance checks must be bound to the consequential mutation/serialization boundary both during startup confirmation and post-construction operations; another unsynchronized `_classify()` merely moves the TOCTOU window.
...
If exact source execution becomes available first, run retained LAB-090 REDs before PR #175 production changes; then run two LAB-092 deterministic TOCTOU RED groups: (1) constructor/restart — pause after initial COMPLETE classification/authority checks but before `confirmation.execute`, mutate marker or activation DDL from a second connection, and require failure before provider position/request-results or durable receipt/intent/meta/watermark/history mutation; (2) post-construction — pause `execute()` immediately after successful reserve/provenance check, delete the confirmed marker from a second connection, resume, and require fail-closed before further external/durable mutation, extending to watermark commit and provider rotation. Then run retained LAB-100 provider REDs.
...
- #176 / LAB-092 — IN_PROGRESS; exact regression/full gate pending; constructor/restart and post-construction provenance TOCTOU regressions are both required.
