...
Completed the recorded distinct fallback: froze `DR_ESCROW_ISSUANCE_KEY_ROTATION_POLICY_LIFECYCLE_V1_FROZEN` in `research/2026-09-05-dr-escrow-issuance-key-rotation-policy-lifecycle-v1.md`, main commit `00919414c59f062889bcbc1b1ef77c97db3d7340`; #178 comment `5548700919` records the result.

Key decisions: independently held escrow authority must be issued before disaster under the current authenticated policy, with quorum plus minimum failure-domain coverage and a provenance commit. New witness/custodian keys cannot self-authorize; rotation is authorized by the old policy and parent-linked into the existing global provenance chain. Historical packages retain their issuance-policy semantics. Threshold/failure-domain downgrade is explicit, old-policy-authorized, non-retroactive, and requires a fresh package + drill before automatic DR readiness. Below-threshold surviving authority or threshold compromise stops automatic rotation for human security ceremony. Policy activation is `DRAFT -> QUORUM_AUTHORIZED -> PROVENANCE_COMMITTED -> DRILL_VERIFIED -> ACTIVE`; divergent successors are `DR_POLICY_SPLIT_VIEW`. Failed/missed drills disable automatic DR readiness but never weaken quorum. A 60-case RED-first matrix is frozen.

Primary donors: TUF v1.0.36 root continuity/threshold key rotation; C2SP transparency-log witness monotonic checkpoint/atomic persistence; NIST SP 800-57 key lifecycle/recovery/trust-anchor guidance.
...
If neither capability appears: next distinct evidence task is to freeze the **DR human security ceremony / re-root authorization contract** for the cases automation must not solve: threshold escrow compromise, no surviving quorum, or irreconcilable split view. Define evidence that must be preserved, explicit human roles/separation of duties, fresh-root generation without trusting disaster-supplied history, independent comparison of surviving pre-disaster anchors, authorization threshold, one-way provenance re-root transition, mandatory post-ceremony full verification/drill, and a hard boundary preventing ordinary admin/provider/worker authority from invoking the ceremony. Compose with LAB-086 public-only history and all frozen DR contracts; do not create a reusable break-glass master secret or a second mutable trust plane.
...
- #178 / LAB-093 — READY; façade/session/request/effect/registry/application-idempotency/install-retention/bounded-capacity/archive/checkpoint/retrieval/lifecycle/archive-loss-DR/escrow-root-recovery/escrow-lifecycle contracts frozen; exact RED/GREEN pending.
