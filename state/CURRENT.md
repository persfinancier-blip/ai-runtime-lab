...
- LAB-094 / #179 READY: immutable provider-history bootstrap trust-root lifetime contract; source compatibility and exact regression-first plan now documented.
...
Continued LAB-094/#179 at source level. Audited `protocol.py`, `integration.py`, `supported.py`, and provider-generation-history test layout. Confirmed the public mutable `bootstrap` slot is consumed as trust authority by both standalone `DurableProviderHistory.verify_durable()` and integrated `_verify_durable_locked()`, while supported wrappers do not override/protect it. Existing inspected tests depend on construction-time bootstrap values but do not require post-construction bootstrap assignment.

Prepared the exact minimal fix contract without staging production code: retain private `_bootstrap`; expose read-only `bootstrap` property; make `_init()`, standalone verify, and integrated locked verify consume the private root; do not use a generalized `__setattr__` freeze and do not mix in path immutability (#180) or caller-owned provider capability changes (#178).

Prepared the exact regression-first security reproduction: construct with legitimate `g1`, build a self-consistent attacker-rooted substituted history that fails under original `g1` but passes pre-fix only after `history.bootstrap = attacker_g1`, then prove post-fix rebinding is impossible and original construction root still rejects substitution. Cover both standalone and supported-ledger integrated verification paths.
...
If execution and LAB-086 byte-preserving publication remain unavailable, continue LAB-094/#179 only far enough to design the concrete self-consistent attacker-history fixture against the exact schema and transition-proof rules; do not stage production code until the pre-fix authority-rebinding acceptance is actually executed or an equivalently strong auditable execution path becomes available. After that fixture plan is complete, move to LAB-095/#180 source-level lifetime-identity audit rather than inventing more LAB-092 wrappers.
...
- #178 / LAB-093 — READY.
- #179 / LAB-094 — READY; source contract + compatibility map + exact regression-first plan complete; executable RED/GREEN and implementation pending exact execution.
- #180 / LAB-095 — READY.
